...
void bind_trajectory(nb::module_& m) {
  ...
  nb::class_<Trajectory>(m, "Trajectory")
          ...
          .def(nb::init<>())
          .def_prop_ro("pos", [](const Trajectory& t) { return matrix_to_copy(t.pos); }, nb::rv_policy::automatic, "Joint positions, shape (n_joints, n_points), Fortran-contiguous float64 array.")
          .def_prop_ro("vel", [](const Trajectory& t) { return matrix_to_copy(t.vel); }, nb::rv_policy::automatic, "Joint velocities, shape (n_joints, n_points), Fortran-contiguous float64 array.")
          .def_prop_ro("acc", [](const Trajectory& t) { return matrix_to_copy(t.acc); }, nb::rv_policy::automatic, "Joint accelerations, shape (n_joints, n_points), Fortran-contiguous float64 array.")
          .def_prop_ro("t", [](const Trajectory& t) { return array_to_copy(t.t); }, nb::rv_policy::automatic, "Time samples, shape (n_points,), float64 array.")
          .def("__repr__", [](const Trajectory& t) { return "<Trajectory n_joints=" + std::to_string(t.pos.rows) +
  ...
  nb::class_<Bspline>(m, "Bspline")
          ...
          .def_prop_ro("traj", [](Bspline& b) -> Trajectory& { return b.traj; }, nb::rv_policy::reference_internal, "Trajectory computed from the last optimize() call.")
          .def("__repr__", [](const Bspline& b) { return "<Bspline n_ctrl=" + std::to_string(b.n_ctrl) +

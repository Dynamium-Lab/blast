...
void bind_optimization(nb::module_& m) {
    ...
    nb::class_<ConstraintSelection>(m, "ConstraintSelection")
        ...
        .def("__repr__", [](const ConstraintSelection& c) {
            ...
            if (c.position)            s += " pos";
            if (c.velocity)            s += " vel";
            if (c.acceleration)        s += " acc";
            if (c.torque)              s += " torque";
            if (c.tool_speed)          s += " tool_speed";
            if (c.self_collisions)     s += " self_col";
            if (c.external_collisions) s += " ext_col";
            return s + ">";
        });
    ...
    nb::class_<Guess>(m, "Guess")
        ...
        .def_prop_rw("initial_x",
            [](Guess& g) {
                if (g.initial_x.size == 0) return nb::ndarray<nb::numpy, real>{};
                return array_to_view(&g.initial_x);
            },
            [](Guess& g, nb::ndarray<nb::numpy, real, nb::ndim<1>, nb::c_contig> a) {
                g.initial_x = array_from_numpy(a);
            },
            "Initial solution vector for GuessType.custom (float64 view).")
        .def("__repr__", [](const Guess& g) {
            return "<Guess type=" + std::to_string((int)g.type) +
    ...
    nb::class_<Result>(m, "Result")
        ...
        .def_prop_ro("x",
            [](Result& r) { return array_to_view(&r.x); },
            "Optimized B-spline control vector (float64 view).")
        .def_prop_ro("x0",
            [](Result& r) { return array_to_view(&r.x0); },
            "Initial guess vector that was used (float64 view).")
        .def_prop_ro("trajectory",
            [](Result& self) -> Trajectory& { return self.trajectory; },
            nb::rv_policy::reference_internal,
            "Computed Trajectory (pos/vel/acc/t). The views it returns keep "
            "the Result alive.")
        .def("__repr__", [](const Result& r) {
            return "<Result success=" + std::string(r.success ? "True" : "False") +
    ...
    // ------------------------------------------------------------------
    m.def("optimize",
        [](Optimization* opt, OptimizationMethod method, u32 output_steps_ms) -> Result {
            nb::gil_scoped_release release;
            return blast::optimize(opt, method, output_steps_ms);
        },
        nb::arg("opt"),
        nb::arg("method")          = OptimizationMethod::with_segments,

...
static nb::list vec3_array_to_list(const std::array<Vec3, N>& a, size_t n) {
    nb::list lst;
    for (size_t i = 0; i < n; i++) lst.append(a[i]);
    return lst;
}
...
void bind_manipulator(nb::module_& m) {
    ...
    nb::class_<ManipulatorLimits>(m, "ManipulatorLimits")
        ...
        .def_prop_rw("position_max",
            [](const ManipulatorLimits& l) { return array_to_copy(l.position_max); },
            [](ManipulatorLimits& l, nb::ndarray<nb::numpy, real, nb::ndim<1>, nb::c_contig> a) {
                l.position_max = array_from_numpy(a);
            }, nb::rv_policy::move, "Maximum joint positions (rad).")
        .def_prop_rw("position_min",
            [](const ManipulatorLimits& l) { return array_to_copy(l.position_min); },
            [](ManipulatorLimits& l, nb::ndarray<nb::numpy, real, nb::ndim<1>, nb::c_contig> a) {
                l.position_min = array_from_numpy(a);
            }, nb::rv_policy::move, "Minimum joint positions (rad).")
        .def_prop_rw("velocity_max",
            [](const ManipulatorLimits& l) { return array_to_copy(l.velocity_max); },
            [](ManipulatorLimits& l, nb::ndarray<nb::numpy, real, nb::ndim<1>, nb::c_contig> a) {
                l.velocity_max = array_from_numpy(a);
            }, nb::rv_policy::move, "Maximum joint velocities (rad/s).")
        .def_prop_rw("acceleration_max",
            [](const ManipulatorLimits& l) { return array_to_copy(l.acceleration_max); },
            [](ManipulatorLimits& l, nb::ndarray<nb::numpy, real, nb::ndim<1>, nb::c_contig> a) {
                l.acceleration_max = array_from_numpy(a);
            }, nb::rv_policy::move, "Maximum joint accelerations (rad/s²).")
        .def_prop_rw("torque_max",
            [](const ManipulatorLimits& l) { return array_to_copy(l.torque_max); },
            [](ManipulatorLimits& l, nb::ndarray<nb::numpy, real, nb::ndim<1>, nb::c_contig> a) {
                l.torque_max = array_from_numpy(a);
            }, nb::rv_policy::move, "Maximum joint torques (N·m).")
        .def_rw("tool_speed_max", &ManipulatorLimits::tool_speed_max,
    ...
    nb::class_<ManipulatorKinematics>(m, "ManipulatorKinematics")
        .def(nb::init<>())
        .def_prop_rw("joint_offsets",
            [](const ManipulatorKinematics& k) {
                return vec3_array_to_list(k.joint_offsets, MAX_JOINTS);
            },
            [](ManipulatorKinematics& k, nb::list lst) {
                size_t i = 0;
                for (auto item : lst) {
        ...
        .def_prop_rw("joint_axes",
            [](const ManipulatorKinematics& k) {
                return vec3_array_to_list(k.joint_axes, MAX_JOINTS);
            },
            [](ManipulatorKinematics& k, nb::list lst) {
                size_t i = 0;
                for (auto item : lst) {
        ...
        .def_prop_rw("static_rotations",
            [](const ManipulatorKinematics& k) {
                return mat3_array_to_list(k.static_rotations, MAX_JOINTS + 1);
            },
            [](ManipulatorKinematics& k, nb::list lst) {
                size_t i = 0;
                for (auto item : lst) {
        ...
        .def_rw("base_position",        &ManipulatorKinematics::base_position,
                "Base position in the world frame (Vec3).")
        .def_rw("first_joint_position", &ManipulatorKinematics::first_joint_position,
                "Position of the first joint relative to the base (Vec3).")
        .def_rw("base_rotation",        &ManipulatorKinematics::base_rotation,
    ...
    nb::class_<ManipulatorDynamics>(m, "ManipulatorDynamics")
        .def(nb::init<>())
        .def_prop_rw("link_masses",
            [](const ManipulatorDynamics& d) {
                return real_array_to_numpy(d.link_masses, MAX_JOINTS);
            },
            [](ManipulatorDynamics& d, nb::ndarray<nb::numpy, real, nb::ndim<1>, nb::c_contig> a) {
                real_array_from_numpy(d.link_masses, a);
            },
            nb::rv_policy::move,
            "Mass of each link (kg). Length must be <= MAX_JOINTS.")
        .def_prop_rw("inertia_tensors",
            [](const ManipulatorDynamics& d) {
                return mat3_array_to_list(d.inertia_tensors, MAX_JOINTS);
            },
            [](ManipulatorDynamics& d, nb::list lst) {
                size_t i = 0;
                for (auto item : lst) {
                    if (i >= (size_t)MAX_JOINTS)
        ...
        .def_prop_rw("cog_offsets",
            [](const ManipulatorDynamics& d) {
                return vec3_array_to_list(d.cog_offsets, MAX_JOINTS);
            },
            [](ManipulatorDynamics& d, nb::list lst) {
                size_t i = 0;
                for (auto item : lst) {
    ...
    nb::class_<ManipulatorCapsules>(m, "ManipulatorCapsules")
        ...
        .def_prop_rw("collision_base",
            [](const ManipulatorCapsules& c) { return array_to_copy(c.collision_base); },
            [](ManipulatorCapsules& c, nb::ndarray<nb::numpy, real, nb::ndim<1>, nb::c_contig> a) {
                c.collision_base = array_from_numpy(a);
            },
            nb::rv_policy::move,
            "Boolean-like array: 1 if capsule[i] can collide with the base sphere.")
        .def_prop_rw("collision_matrix",
            [](const ManipulatorCapsules& c) -> nb::ndarray<nb::numpy, uint8_t> {
                ...
                return nb::ndarray<nb::numpy, uint8_t>(buf, 2, shape, owner, strides);
            },
            [](ManipulatorCapsules& c, nb::ndarray<nb::numpy, uint8_t, nb::ndim<2>> a) {
                int rows = (int)a.shape(0), cols = (int)a.shape(1);
                c.collision_matrix.resize(rows, cols);
                for (int r = 0; r < rows; r++)
                    for (int col = 0; col < cols; col++)
                        c.collision_matrix(r, col) = a(r, col);
            },
            nb::rv_policy::move,
            "Symmetric adjacency matrix (uint8): collision_matrix[i,j]=1 means capsule i and j can collide.");
    ...
    nb::class_<Tool>(m, "Tool")
        ...
        .def_prop_rw("collision_matrix",
            [](const Tool& t) -> nb::ndarray<nb::numpy, uint8_t> {
            ...
            [](Tool& t, nb::ndarray<nb::numpy, uint8_t, nb::ndim<2>> a) {
                int rows = (int)a.shape(0), cols = (int)a.shape(1);
                t.collision_matrix.resize(rows, cols);
                for (int r = 0; r < rows; r++)
                    for (int col = 0; col < cols; col++)
                        t.collision_matrix(r, col) = a(r, col);
            },
            nb::rv_policy::move);
    ...
    nb::class_<Manipulator>(m, "Manipulator")
        ...
        .def("__init__",
             [](Manipulator* self, u32 n, const ManipulatorLimits& l,
                const ManipulatorKinematics& k, const ManipulatorDynamics& d) {
                 new (self) Manipulator(n, l, k, &d, nullptr);
             },
             nb::arg("n_joints"), nb::arg("limits"), nb::arg("kinematics"), nb::arg("dynamics"),
             "Construct with kinematics and dynamics.")
        .def("__init__",
             [](Manipulator* self, u32 n, const ManipulatorLimits& l,
                const ManipulatorKinematics& k, const ManipulatorDynamics& d,
                const ManipulatorCapsules& c) {
                 new (self) Manipulator(n, l, k, &d, &c);
             },
             nb::arg("n_joints"), nb::arg("limits"), nb::arg("kinematics"),
             nb::arg("dynamics"), nb::arg("capsules"),
             "Construct with kinematics, dynamics, and collision capsules.")
        ...
        .def("set_payload", &Manipulator::set_payload,
             nb::arg("mass"), nb::arg("cog"), nb::arg("inertia"))
        ...
        .def("__repr__", [](const Manipulator& manip) {
            return "<Manipulator n_joints=" + std::to_string(manip.n_joints) + ">";
        });
}

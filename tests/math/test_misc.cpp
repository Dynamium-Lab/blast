...
TEST_CASE("wrap2pi() - scalar wrapping", "[Math][Misc]") {
  using namespace blast;
  CHECK(std::abs(wrap2pi(1.0) - 1.0) < 1e-9);
  CHECK(std::abs(wrap2pi(-1.0) - (-1.0)) < 1e-9);
  CHECK(std::abs(wrap2pi(0.0) - 0.0) < 1e-9);
  CHECK(std::abs(wrap2pi(4.0) - (4.0 - 2 * PI)) < 1e-9);
  CHECK(std::abs(wrap2pi(-4.0) - (-4.0 + 2 * PI)) < 1e-9);
}
...
TEST_CASE("wrap_to_180() - real and float overloads", "[Math][Misc]") {
  using namespace blast;
  CHECK(std::abs(wrap_to_180(90.0) - 90.0) < 1e-9);
  CHECK(std::abs(wrap_to_180(270.0) - (-90.0)) < 1e-9);
  CHECK(std::abs(wrap_to_180(-270.0) - 90.0) < 1e-9);
...
TEST_CASE("deg2rad() / rad2deg() - scalar", "[Math][Misc]") {
  using namespace blast;
  CHECK(std::abs(deg2rad(0.0) - 0.0) < 1e-12);
  CHECK(std::abs(deg2rad(180.0) - PI) < 1e-9);
  CHECK(std::abs(deg2rad(90.0) - PI / 2) < 1e-9);
  CHECK(std::abs(rad2deg(0.0) - 0.0) < 1e-12);
  CHECK(std::abs(rad2deg(PI) - 180.0) < 1e-9);
  CHECK(std::abs(rad2deg(PI / 2) - 90.0) < 1e-9);
  CHECK(std::abs(rad2deg(deg2rad(45.0)) - 45.0) < 1e-9);
  CHECK(std::abs(deg2rad(rad2deg(1.0)) - 1.0) < 1e-9);
}
...
TEST_CASE("deg2rad() / rad2deg() - Array overloads", "[Math][Misc]") {
  ...
  CHECK(std::abs(angles_rad[0] - 0.0) < 1e-9);
  CHECK(std::abs(angles_rad[1] - PI / 2) < 1e-9);
  CHECK(std::abs(angles_rad[2] - PI) < 1e-9);
  CHECK(std::abs(angles_rad[3] - 2 * PI) < 1e-9);
  Array back = rad2deg(angles_rad);
  CHECK(std::abs(back[0] - 0.0) < 1e-9);
  CHECK(std::abs(back[1] - 90.0) < 1e-9);
  CHECK(std::abs(back[2] - 180.0) < 1e-9);
  CHECK(std::abs(back[3] - 360.0) < 1e-9);
}
...
TEST_CASE("rpy2rotation() - identity angles produce identity matrix", "[Math][Misc]") {
  ...
  CHECK(is_close(R, expected, 1e-12));
}
...
TEST_CASE("rpy2rotation() / rotation2rpy() - explicit roundtrip", "[Math][Misc]") {
  ...
  CHECK(is_close(rpy_in, rpy_out, 1e-9));
}

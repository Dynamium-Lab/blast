...
TEST_CASE("UR5e forward_kinematics() test", "[Generic]") {
    ...
    //  @ home position
    Vec3 expected_tcp_position = {0.0, -0.233, 1.08};
    Array test_position = {0.0, -PI/2, 0.0, -PI/2, 0.0, 0.0};
    forward_kinematics(generic_manip, test_position);
    CHECK(is_close(generic_manip._p_j[generic_manip.joints], expected_tcp_position, 1e-2));

    //  @ 2nd position
    Vec3 expected_tcp_position_2 = {-0.1179, -0.436, 0.152};
    Array test_position_2 = {-1.54, -1.83, -2.28, -0.59, 1.60, 0.023};
    forward_kinematics(generic_manip, test_position_2);
    CHECK(is_close(generic_manip._p_j[generic_manip.joints], expected_tcp_position_2, 1e-2));
}

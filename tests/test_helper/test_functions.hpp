...
inline host_fn bool is_close(const Manipulator& manip1, const Manipulator& manip2, real eps = 1e-5) {
  ...
  if (!is_close(manip1.dv, manip2.dv, eps))
    return false; // vector to next joint
  if (!is_close(manip1.ev, manip2.ev, eps))
  ...
  if (!is_close(manip1.av, manip2.av, eps))
    return false; // center of mass
  if (!is_close(manip1.sv, manip2.sv, eps))
  ...
  if (!is_close(manip1._collision_model, manip2._collision_model, eps))
    return false;
...
inline host_fn bool is_close(const ConstraintSelection& constraints1, const ConstraintSelection& constraints2, real eps = 1e-5) {
  ...
  if (constraints1.external_collisions != constraints2.external_collisions)
    return false;
  if (!is_close(constraints1.n_collision_constraints, constraints2.n_collision_constraints, eps))
    return false;
  if (!is_close(constraints1.n_collision_skip, constraints2.n_collision_skip, eps))
    return false;
  if (!is_close(constraints1.n_constraints, constraints2.n_constraints, eps))
    return false;

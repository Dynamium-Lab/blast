...
TEST_CASE("Test gradient", "[accelerations]") {
  ...
  opt.guess.x0   = Array{0.152355, -0.597958, -0.756348, 0.558375, -0.946557, -0.10268, 0.901938, -0.145201, -0.436917, -0.827993,
  ...
  opt_acc.guess.x0   = Array{0.152355, -0.597958, -0.756348, 0.558375, -0.946557, -0.10268, 0.901938, -0.145201, -0.436917, -0.827993,
                           -0.210473, -0.0664338, 0.370776, -0.29955, -0.139142, 0.757138, -0.157328, 0.909626, -0.214084, 0.745555,
                           -0.0416009, 0.375275, 0.126153, 0.402337, 0.723458, -0.070362, -0.444889, 0.784517, 0.655188, 0.216073,
                           0.889643, -0.969162, -0.0279569, 0.496376, -0.891977, 0.0247388, -0.332795, -0.239096, -0.418835, 0.922935,
                           0.124817, 0.34831, -0.00905602, -0.157159, 0.492551, 0.0325292, -0.299891, 0.828649, -0.909505, -0.294613,
  ...
  if (opt_acc.constraints.torque) {
    ...
    for (int j = 0; j < joints; j++) {
      torque_constraint[j] = abs_constraint(1.01 * manip_data.efforts[j], opt_acc.manip.tau_max[j]);
    }
    ...
    for (u32 j = 0; j < joints; j++) {
      ...
      for (u32 k = 0; k < joints; k++) {
        torque_constraint_plus[k]                     = abs_constraint(1.01 * manip_data.efforts[k], opt_acc.manip.tau_max[k]);
  ...
  for (u32 j = 0; j < xlen - 1; j++) {              // last one is T todo: maybe change to 2 for loops (joint & x_per_joint)
    ...
    Array external_collisions(opt_acc.bspline.ub[x_idx] - opt_acc.bspline.lb[x_idx]);
    ...
    for (u32 i = opt_acc.bspline.lb[x_idx]; i <= opt_acc.bspline.ub[x_idx]; i++) { // lb & ub are inclusive
      grad_idx += joint * xlen;
...
TEST_CASE("Test gradient accuracy", "[accelerations]") {
  ...
  //   opt_acc_2.guess.x0   = Array{0.152355, -0.597958, -0.756348, 0.558375, -0.946557, -0.10268, 0.901938, -0.145201, -0.436917, -0.827993,
  //                              -0.210473, -0.0664338, 0.370776, -0.29955, -0.139142, 0.757138, -0.157328, 0.909626, -0.214084, 0.745555,
  //                              -0.0416009, 0.375275, 0.126153, 0.402337, 0.723458, -0.070362, -0.444889, 0.784517, 0.655188, 0.216073,
  //                              0.889643, -0.969162, -0.0279569, 0.496376, -0.891977, 0.0247388, -0.332795, -0.239096, -0.418835, 0.922935,
  //                              0.124817, 0.34831, -0.00905602, -0.157159, 0.492551, 0.0325292, -0.299891, 0.828649, -0.909505, -0.294613,

    ...
    for(u32 t = 0; t < n_tests; t++) {
        ...
        for (u32 i = 0; i < nctrl; i++) {
            ...
            for (u32 k = lb; k <= ub; k++) {
                ...
                bool b_v = bspline.basis_v(i, k) != 0.0 ? true : bspline.basis_v(i-1, k) != 0.0 && bspline.basis_v(i+1, k) != 0.0 ? true : false; // remove outliers
                bool b_a = bspline.basis_a(i, k) != 0.0 ? true : bspline.basis_a(i-1, k) != 0.0 && bspline.basis_a(i+1, k) != 0.0 ? true : false; // remove outliers
                CHECK(b_p);
                CHECK(b_v);
                CHECK(b_a);
            }

...
int main() {
    ...
    EigenPvaBspline pva_eigen(nctrl, npts, p, njoints);
    ...
    Eigen::MatrixXd task_eigen(njoints, 6);
    ...
    for (u32 i = 0; i < x.size; i++)
        x[i] = amp * get_random();
    x[x.size-1] = 0.9f;
    Eigen::VectorXd x_eigen(pva.xlen());
    for (u32 i = 0; i < x.size; i++)
        x_eigen[i] = x[i];
    ...
    pva_eigen.compute_trajectory(x_eigen, task_eigen);
    for (u32 i = 0; i < njoints; i++) {
        // boundary conditions
        init_max_pos_error  = std::max(init_max_pos_error,  std::abs((double)pva.pos(i, 0) - (double)task(i, 0)));
        init_max_vel_error  = std::max(init_max_vel_error,  std::abs((double)pva.vel(i, 0) - (double)task(i, 1)));

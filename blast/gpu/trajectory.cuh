...
struct cuMultiBsplines {
  ...
  host_fn cuMultiBsplines(const cuMultiBsplines& other) {
    *this    = other;
    is_alias = true;
  }
...
host_fn void cuMultiBsplines::compute_trajectories(const Array& x, const Matrix& task) {
  ...
  compute_trajectories_kernel<<<ntrajectories, points>>>();
  cuda_check_kernel;
...
host_fn cuMultiBsplines::~cuMultiBsplines() {
  if (!is_alias) {
    cuda_check(cudaFreeHost(pos));
    pos = nullptr;
    cuda_check(cudaFreeHost(vel));
    vel = nullptr;
    cuda_check(cudaFreeHost(acc));
    acc = nullptr;
    cuda_check(cudaFreeHost(t));
    t = nullptr;
    cuda_check(cudaFreeHost(control));
    control = nullptr;
    cuda_check(cudaFreeHost(dt));
    dt = nullptr;
    cuda_check(cudaFreeHost(one_over_T));
    one_over_T = nullptr;
    cuda_check(cudaFreeHost(one_over_T2));
    one_over_T2 = nullptr;
    delete host;
    host = nullptr;
    cuda_check(cudaFree(dev_basis_p));
    dev_basis_p = nullptr;
    cuda_check(cudaFree(dev_basis_v));
    dev_basis_v = nullptr;
    cuda_check(cudaFree(dev_basis_a));
    dev_basis_a = nullptr;
    cuda_check(cudaFree(dev_pos));
    dev_pos = nullptr;
    cuda_check(cudaFree(dev_vel));
    dev_vel = nullptr;
    cuda_check(cudaFree(dev_acc));
    dev_acc = nullptr;
    cuda_check(cudaFree(dev_t));
    dev_t = nullptr;
    cuda_check(cudaFree(dev_control));
    dev_control = nullptr;
    cuda_check(cudaFree(dev_dt));
    dev_dt = nullptr;
    cuda_check(cudaFree(dev_one_over_T));
    dev_one_over_T = nullptr;
    cuda_check(cudaFree(dev_one_over_T2));
    dev_one_over_T2 = nullptr;
  }
}

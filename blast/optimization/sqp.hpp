...
inline void print_nlopt_stopping(const nlopt_stopping* s, const char* name) {
  if (!s) {
...
inline void lsq_(int* m, int* meq, int* n, int* nl,
  ...
  /* SGJ, 2010: skip constraints for infinite bounds */
  for (i__ = 1; i__ <= *n; ++i__) {
    w[(il - 1) + i__] = isinf(xl[i__]) ? 0 : xl[i__];
    w[(iu - 1) + i__] = isinf(xu[i__]) ? 0 : -xu[i__];
  }
...
inline char* nlopt_vsprintf(char* p, const char* format, va_list ap) {
...
inline void nlopt_stop_msg(const nlopt_stopping* s, const char* format, ...) {
  va_list ap;
  if (s->stop_msg) {
    va_start(ap, format);
...
inline unsigned nlopt_count_constraints(unsigned p, const nlopt_constraint* c) {
  unsigned i, count = 0;
  for (i = 0; i < p; ++i)
    count += c[i].m;
  return count;
...
inline unsigned nlopt_max_constraint_dim(unsigned p, const nlopt_constraint* c) {
...
L240:
  ...
  if (((d__1 = *f - f0, fabs(d__1)) < *acc || dnrm2___(n, &s[1], 1) < *acc) && h3 < *acc) {
    *mode = 0;
  } else {
    *mode = -1;
  }
...
L255:
  if (((d__1 = *f - f0, fabs(d__1)) < tol || dnrm2___(n, &s[1], 1) < tol) && h3 < tol) {
    *mode = 0;
  } else {
    *mode = 8;
  }
...
inline void slsqp(int* m, int* meq, int* la, int* n,
  ...
  n1    = *n + 1;
  mineq = *m - *meq + n1 + n1;
  il    = (n1 * 3 + *m) * (n1 + 1) + (n1 - *meq + 1) * (mineq + 2) + (mineq << 1) + (n1 + mineq) * (n1 - *meq) + (*meq << 1) + n1 * *n / 2 + (*m << 1) + *n * 3 + (n1 << 2) + 1;
  /* Computing MAX */

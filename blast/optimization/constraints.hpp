...
struct Constraints {
    ...
    std::vector<void (*)(double* result, Optimization<T_manip>* opt)> extra_constraints = {};
    std::vector<u32>             n_extra_constraints = {};
    void add_constraint(void (*constraint)(double* result, Optimization<T_manip>* opt), int n_con);
};

template <typename T_manip>
void Constraints<T_manip>::add_constraint(void (*constraint)(double* result, Optimization<T_manip>* opt), int n_con) {
    extra_constraints.push_back(constraint);
    n_extra_constraints.push_back(n_con);
}

inline real bound_constraint(const real& q, const real& q_min, const real& q_max) {
    real g1 = q_max ==  INF_REAL ? -1.0 : q_max == 0.0 ?  q : sign(q_max) * (q / q_max - 1);
    real g2 = q_min == -INF_REAL ? -1.0 : q_min == 0.0 ? -q : sign(q_min) * (1 - q / q_min);
    return g1 > g2 ? g1 : g2;
}

// todo: replace by jacobian?
template <typename T_manip>
inline Matrix get_J_tool(const Optimization<T_manip>* opt) {
    std::vector<Vec3> r_tool(opt->manip.joints);
    r_tool[opt->manip.joints-1] = opt->manip.dv[opt->manip.joints-1];
    for (int i = opt->manip.joints - 2; i >= 0; i--) {
        r_tool[i] = opt->manip.dv[i] + opt->manip._Q[i+1] * r_tool[i+1];
    }

    for (int i = 0; i < opt->manip.joints; i++) {
        r_tool[i] = opt->manip._Q_mult[i] * r_tool[i];
    }
    ...
    for (int i = 0; i < opt->manip.joints; i++) {
        ...
        cr_tool = cross(opt->manip._Q_mult[i] * opt->manip.ev[i], r_tool[i]);
...
template <typename T_manip>
inline void compute_constraints(double* result, const Array& x, Optimization<T_manip>* opt) {
    double* moving_result = result;

    opt->bspline.compute_trajectory(x, opt->task);

    ObjMatrix<Capsule> capsules(opt->manip._n_caps, (u32)opt->bspline.points/opt->constraints.n_collision_skip);
    int test_idx = 0; // todo: Comment this out

    // {
    //     blast_time_block("Points Constraints");
    for (u32 i = 0; i < opt->bspline.points; i++) {
        ...
        if (opt->constraints.external_collisions) {
            if (i % opt->constraints.n_collision_skip == 0 && i != opt->bspline.points - 1) { // todo: this cleaner
                for (u32 j = 0; j < opt->manip._n_caps; j++) {
                    capsules(j, i/opt->constraints.n_collision_skip) = opt->manip._capsule_list[j];
                }
            }
        }
...
template <typename T_manip>
inline void nlopt_constraints(unsigned m, double *result, unsigned xlen, const double* x, double* grad, void* f_data) {
...
template <typename T_manip>
inline bool validate_task(Optimization<T_manip>* opt) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (opt->constraints.external_collisions || opt->constraints.self_collisions) {
            ...
            if (opt->constraints.self_collisions) {
                auto tmp_coll = manip.internal_collisions();
                if (min(tmp_coll) < 0) { // min because collisions constraints are -d < 0

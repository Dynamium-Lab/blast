...
// Array that does not own it's data (will not free internal memory on destruction)
struct ArrayAlias : Array {
    // unallowed functions
    ArrayAlias(u32 new_size) = delete;
    ArrayAlias(const Matrix&) = delete;
    ArrayAlias(const Array&) = delete;
    ArrayAlias(Array&&) = delete;
    void resize(u32 new_size) = delete;

    // new or modified

    ArrayAlias(Array&);         // shadow constructor
    ArrayAlias(Matrix& m);      // interpret matrix as a flat array
...
// print a Vec3 to console on one line
void print(Vec3);
// print a Mat3 to console on three lines
void print(Mat3);
// print Array to console on one line
void print(Array&);
// print Matrix to console on as many lines as it takes
void print(Matrix&);
...
inline real array_max(Array& a) {
    ...
    for(u32 i = 0; i < a.size; i++)
        result = a[i] > result ? a[i] : result;
    return result;
inline Vec3& operator+=(Vec3& v, Vec3&rhs) {
    v.x += rhs.x;
    v.y += rhs.y;
    v.z += rhs.z;
    return v;
...
inline real two_segment_distance_sqr(Vec3 P0, Vec3 P1, Vec3 Q0, Vec3 Q1) {
    ...
    auto const Q1mQ0 = Q1 - Q0;
       ...
       // note: ada pted from https://www.geometrictools.com/GTE/Mathematics/DistSegmentSegment.h
inline r        eal two_segment_distance_sqr(Vec3 P0, Vec3 P1, Vec3 Q0, Vec3 Q1) {
            auto const P1mP0 = P1 - P0;
            auto const Q1mQ0 = Q1 - Q0;
    ...
    Vec3 clo        sest0 = P0 + parameter[0]*P1mP0;
         Vec3    closest1 = Q0 + parameter[1]*Q1mQ0;
            Vec3 diff = closest0 - closest1;

...
Optim_solution collision_pso_data(const Matrix &caps_list, const OBB &OBB, const int N_particles, const int N_iterations) {
    ...
    solution.best_f = gbest_f;
    solution.best_x = gbest_x;
    return solution;
}
...
Optim_solution test_collision_pso_OBB_data(const Matrix &cart_pos, const  objlist* world, const int N_particles, const int N_iterations) {
    ...
    int n_points = cart_pos.cols;
    Optim_solution solution;
    solution.best_f = INF_REAL;
    Optim_solution temp_dist;
    Matrix temp(6, n_points);
    // for (int j = 0; j < n_caps; j++) {
    for (int j = 0; j < n_caps; j++) {
        for (int i = 0; i < n_points; i++) {
        ...
        for (int i = 0; i < world->OBBlist.size(); i++) {
            temp_dist = collision_pso_data(temp, world->OBBlist[i], N_particles, N_iterations);
            solution.caps   = (temp_dist.best_f < solution.best_f) ? j : solution.caps;
            solution.best_x = (temp_dist.best_f < solution.best_f) ? temp_dist.best_x : solution.best_x;
            solution.best_f = (temp_dist.best_f < solution.best_f) ? temp_dist.best_f : solution.best_f;
        }
    }
    return solution;
}

...
struct EigenPvaBspline {
    Eigen::MatrixXd pos; // njoints x npoints
    Eigen::MatrixXd vel; // njoints x npoints
    Eigen::MatrixXd acc; // njoints x npoints
    Eigen::VectorXd t; // npoints

    Eigen::Matrix<double, -1, -1, Eigen::RowMajor> control; // njoints x ncrtl
    Eigen::MatrixXd basis_p; // nctrl x npoints
    Eigen::MatrixXd basis_v; // nctrl x npoints
    Eigen::MatrixXd basis_a; // nctrl x npoints
    ...
    EigenPvaBspline(u32 ncontrol, u32 npoints, u32 P, u32 dof);
    void compute_trajectory(Eigen::VectorXd &x, Eigen::MatrixXd& task);
    u32 xlen();
    void compute_basis();
    void compute_control(Eigen::VectorXd &x, Eigen::MatrixXd& task);
};
...
inline EigenPvaBspline::EigenPvaBspline(u32 ncontrol, u32 npoints, u32 P, u32 dof) :
    pos(dof, npoints),
    vel(dof, npoints),
    acc(dof, npoints),
    t(npoints),
...
inline void EigenPvaBspline::compute_basis() {
    u32 m = nctrl + p;
    Array knots(m+1);
    {
        for (u32 i = m; i > m-p-1; i--)
...
inline u32 EigenPvaBspline::xlen() {
    return joints*(nctrl-6)+1;
}

inline void EigenPvaBspline::compute_trajectory(Eigen::VectorXd &x, Eigen::MatrixXd& task) {
    Assert(x.size() == xlen());
    Assert(task.rows() == joints);
    Assert(task.cols() == 6);
    ...
    pos = control * basis_p;
    vel = (control * basis_v) * one_over_T;
    acc = (control * basis_a) * one_over_T2;
...
inline void EigenPvaBspline::compute_control(Eigen::VectorXd &x, Eigen::MatrixXd& task) {
    const real T = x[x.size()-1];
    const real du = 1.0f/(nctrl-p);
    const real T2 = T*T;
    const real one_over_T = 1/T;

...
inline host_fn void World::add_box(const Box& box) {
  Assert(is_close(norm(box.rotation.col_copy(0)), 1.0));
  Assert(is_close(norm(box.rotation.col_copy(1)), 1.0));
  Assert(is_close(norm(box.rotation.col_copy(2)), 1.0));
  Assert(is_close(norm(Vec3(box.rotation(0, 0), box.rotation(0, 1), box.rotation(0, 2))), 1.0));
  Assert(is_close(norm(Vec3(box.rotation(1, 0), box.rotation(1, 1), box.rotation(1, 2))), 1.0));
  Assert(is_close(norm(Vec3(box.rotation(2, 0), box.rotation(2, 1), box.rotation(2, 2))), 1.0));

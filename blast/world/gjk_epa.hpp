...
host_fn gjkresult GJK_solve_gjk_simple(Capsule caps, Box box) {
  ComplexSimplex simplex;
  gjkresult      results;
  ...
  while (true) {
    ...
    if (dot(p, p) < COLLISION_EPSILON && old_simplex_count == 4) {
      ...
      results.A_closept        = {};
      results.B_closept        = {};
      results.minimal_distance = solve_EPA_algorithm(simplex, v1, v2) - caps.radius;
      Assert(isnan(results.minimal_distance) == false);
      break;
    }
    ...
    if (current_highest - goal_post <= COLLISION_EPSILON) {
      ...
      results.B_closept        = local.p2;
      results.final_simplex    = simplex;
      results.minimal_distance = norm(results.A_closept - results.B_closept) - caps.radius;
      Assert(isnan(results.minimal_distance) == false);
      break;
    }

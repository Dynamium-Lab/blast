...
inline Matrix::Matrix(u32 r, u32 c, real value) {
  size = r * c;
  rows = r;
  cols = c;
  if (size) {
...
inline blast_fn Matrix::Matrix(const Matrix& m) :
    size(m.size),
    cols(m.cols),
    rows(m.rows),
    is_alias(false) {
...
inline blast_fn Matrix::Matrix(Matrix&& m) noexcept :
    data(m.data),
    size(m.size),
    cols(m.cols),
    rows(m.rows),
...
inline blast_fn Matrix::Matrix(const Array& v) :
    size(v.size),
    cols(1),
    rows(v.size),
    is_alias(false) {

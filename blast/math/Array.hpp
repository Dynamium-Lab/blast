...
// Member functions
inline blast_fn Array::Array(u32 new_size) :
    size(new_size) {
  if (size) {
...
inline blast_fn Array::Array(u32 n, real value) :
    size(n) {
  if (n) {
    data = (real*) Malloc(ALIGN, size * sizeof(real));
    for (u32 i = 0; i < n; i++)
...
inline blast_fn Array::Array(const Array& a) :
    size(a.size) {
  if (size) {
    data = (real*) Malloc(ALIGN, size * sizeof(real));
...
inline blast_fn Array::Array(Array&& a) noexcept :
    data(a.data),
    size(a.size),
    is_alias(a.is_alias) {
  a.data     = nullptr;

...
//      h                        : real containing height above pick-place point  //Vec3 containing height, velocity, and acceleration of intermediate point (above pick-up zone)
...
std::vector<Manipulator_state> pick_and_place(Link6_dev &manip, const Manipulator_state &Current_manip_state, const Matrix &Obj_list, const Matrix Drop_box_list, const real h) {
    ...
    // auto v = hva.v;
    // auto a = hva.a;
    std::vector<Manipulator_state> manip_list;
    ...
    manip_state.pos = Current_manip_state.pos;
    manip_state.vel = Current_manip_state.vel;
    manip_state.acc = Current_manip_state.acc;
    ...
    Array joint_acc(N_joints, 0.0);
    Array joint_vel(N_joints, 0.0);
    ...
    for (int i = 0; i < N_objects; i++) {
        ...
        joint_pos2 = manip.inverse_kinematics(current_obj, manip_list.back().pos);
        joint_pos1 = manip.inverse_kinematics(current_obj + h, joint_pos2);

        Array current_dropbox = Drop_box_list.col(drop_box_id);
        joint_pos3 = manip.inverse_kinematics(current_dropbox + h, joint_pos2);
        joint_pos4 = manip.inverse_kinematics(current_dropbox, joint_pos3);
        ...
        //     joint_vel4[j] = 0;
        // }
        // joint_vel1 = pinv_svd(manip.jacobian(joint_pos1))*v; // todo: check if pinv_svd better than pinv
        // joint_vel3 = pinv_svd(manip.jacobian(joint_pos3))*v;
        ...
        manip_state.pos = joint_pos1;
        manip_state.vel = joint_vel; // 0.0
        manip_state.acc = joint_acc; // 0.0
        manip_state.gripper_state = 0;

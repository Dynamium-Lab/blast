...
static void BM_Splines_Eigen(benchmark::State& state) {
    // init code here
    ...
    // random optimization vector
    Eigen::VectorXd x(njoints*(nctrl-6) + 1);
    for (u32 i = 0; i < x.size(); i++)
        x[i] = amp * get_random();
    x[x.size()-1] = std::abs(x[x.size()-1]);

    // Compute basis functions
    EigenPvaBspline pva(nctrl, npts, p, njoints);
...
BENCHMARK(BM_Splines_Eigen)->Unit(benchmark::kMicrosecond);
